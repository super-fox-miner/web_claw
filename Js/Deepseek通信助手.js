...
(function() {
    ...
    function createButtonPanel() {
        ...
        header.innerHTML = `
            <span class="panel-title">🤖 DeepSeek 助手</span>
    ...
    function getInputElement() {
        let input = document.querySelector('textarea[placeholder*="message" i], textarea[placeholder*="发送" i], div[contenteditable="true"][role="textbox"]');
        if (!input) {
    ...
    function sendMessage() {
        ...
        setTimeout(() => {
            const sendButton = document.querySelector(
                ...
                'button[class*="send"], button[class*="Send"], .ds-btn-primary, button[type="submit"], ' +
    ...
    function getLastAssistantMessage() {
        ...
        for (const sel of selectors) {
            ...
            if (elements.length > 0) {
                const message = elements[elements.length - 1].innerText || '';
                if (message) {
                    return message;
                }
            }
    ...
    function setupAnswerObserver() {
        ...
        const container = document.querySelector('.chat-messages, .ds-chat-messages, .flex-col.items-center, main') || document.body;
    ...
    function setupSendInterceptors() {
        ...
        document.addEventListener('click', function(e) {
            ...
            const sendButton = e.target.closest(
                'button[aria-label="发送消息"], button[aria-label="Send message"], ' +
                'button[class*="send"], button[class*="Send"], .ds-btn-primary, ' +
                'button[class*="chat"]:not([disabled])'
            );
    ...
    async function init() {
        log.info('='.repeat(50));
        log.info('开始初始化 DeepSeek 助手 (双向通信版)');
        log.info('='.repeat(50));
